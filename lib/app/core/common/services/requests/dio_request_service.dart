...
import 'package:geek_app/app/core/common/services/requests/errors/not_internet_connection_exception.dart';
import 'package:geek_app/app/core/common/services/requests/request_result.dart';
...
class DioRequestService extends RequestService {
  ...
  Future<RequestResult> put(
    ...
    try {
      ...
      var response = await _dio.put(url, data: data, options: Options(headers: headers));
      if ((response.statusCode ?? 404) >= 200 && (response.statusCode ?? 404) < 300) {
        return RequestResult(
          statusCode: response.statusCode ?? 0,
          data: response.data,
  ...
  Future<RequestResult> patch(
    ...
    try {
      ...
      var response = await _dio.patch(url, data: data, options: Options(headers: headers));
      if ((response.statusCode ?? 404) >= 200 && (response.statusCode ?? 404) < 300) {
        return RequestResult(
          statusCode: response.statusCode ?? 0,
          data: response.data,
